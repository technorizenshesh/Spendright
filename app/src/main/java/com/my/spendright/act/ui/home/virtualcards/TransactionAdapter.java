...
public class TransactionAdapter extends RecyclerView.Adapter<TransactionAdapter.MyViewHolder> {
   Context context;
    ...
    public TransactionAdapter(Context context,ArrayList<GetExpenSeReport.Result> modelList) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
       ...
       if(modelList.get(position).getFlag().equalsIgnoreCase("AMOUNT_TRANSFERRED_TO_OTHER_USER_IN_MAIN_WALLET") || modelList.get(position).getFlag().equalsIgnoreCase("WIDRAW_MONEY_FROM_MAIN_WALLET_TO_BANK"))
        ...
        holder.binding.txtPrice.setText("₦"+Preference.doubleToStringNoDecimal(Double.parseDouble(FinalAmt.replace(",",""))));
